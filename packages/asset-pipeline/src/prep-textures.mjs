...
const VENDOR_MAP = {
  metal: { id: 'acg-metal-006', zip: 'Metal006_1K-JPG.zip' },
  wood: { id: 'acg-wood-floor-043', zip: 'WoodFloor043_1K-JPG.zip' },
  concrete: { id: 'acg-concrete-040', zip: 'Concrete040_1K-JPG.zip' },
  rubber: { id: 'acg-rubber-004', zip: 'Rubber004_1K-JPG.zip' },
};
 ...
 */
async function packMR(destDir, roughPath, metalPath) {
...
async function genGrip(destDir) {
  ...
  for (let y = 0; y < size; y++) {
    for (let x = 0; x < size; x++) {
      ...
      const val = clamp(0.02 + speck * 0.18 + f * 0.03, 0, 1);
      const i = (y * size + x) * 3;
      const v8 = Math.round(val * 255);
      albedo[i] = v8; albedo[i + 1] = v8; albedo[i + 2] = v8;
      // roughness: very high, grit slightly varies it
      const r = clamp(0.9 + (g - 0.5) * 0.12, 0.82, 0.99);
...
async function genDeckGraphic(destDir) {
  ...
  // gradient + abstract chevron banding + faint grain.
  const cA = [0.09, 0.11, 0.16]; // deep slate
  const cB = [0.16, 0.42, 0.45]; // teal
  const cAccent = [0.85, 0.55, 0.2]; // warm amber
...
async function genUrethane(destDir) {
  ...
  for (let y = 0; y < size; y++) {
    for (let x = 0; x < size; x++) {
      const r = clamp(0.32 + (noise(x * 0.05, y * 0.05) - 0.5) * 0.14, 0.2, 0.5);
      const i = (y * size + x) * 3;
...
export async function prepTextures() {
  ...
  for (const logical of Object.keys(VENDOR_MAP)) {
    ...
    const mr = await packMR(destDir, byKind.Roughness, byKind.Metalness || null);
    manifest[logical] = {
